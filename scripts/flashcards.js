...
function renderTabs() {
  ...
  for (const [id, deck] of Object.entries(decks)) {
    ...
    btn.innerHTML = `${deck.label}<span class="count">${due} due</span>`;
...
function renderCard(card, pick) {
  ...
  areaEl.innerHTML = `
    ...
    <div class="rate-row" id="rate-row" style="opacity:0.45; pointer-events:none;">
      ...
      <button class="rate-btn good"  data-q="4">Good<span class="key">3</span></button>
      <button class="rate-btn easy"  data-q="5">Easy<span class="key">4</span></button>
    </div>
  `;
  metaEl.innerHTML = pick.mode === "due"
    ? `${pick.dueCount} cards due in this deck`
    : `Nothing due — reviewing ahead`;
