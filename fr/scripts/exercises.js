...
const TYPE_LABELS = {
  translate: "Traduis en chinois",
  recognize: "Choisis le sens",
  pinyin:    "Écris le pinyin",
  syntax:    "Construis la phrase",
};

const FILTERS = [
  { id: "all",       label: "Tous" },
  { id: "translate", label: "Traduction" },
  { id: "recognize", label: "Reconnaissance" },
  { id: "pinyin",    label: "Pinyin" },
  { id: "syntax",    label: "Syntaxe" },
];
...
function renderScore() {
  ...
  const human = total ? `Question ${clampPos(pos) + 1} sur ${total}` : "Aucun exercice dans ce filtre";
    ...
    mount(ex, host) {
      host.innerHTML = `
        <div class="answer-row">
          ...
          <button class="btn btn-accent" data-action="check" type="button">Vérifier</button>
          <button class="btn btn-ghost" data-action="show" type="button">Voir la réponse</button>
        </div>
      `;
  ...
  pinyin: {
    mount(ex, host) {
      host.innerHTML = `
        ...
        <div class="answer-row">
          <input type="text" class="answer-input pinyin-input" placeholder="écris le pinyin (ā á ǎ à ou wo3)…" autocomplete="off" autocorrect="off" autocapitalize="off" spellcheck="false" />
          <button class="btn btn-accent" data-action="check" type="button">Vérifier</button>
          <button class="btn btn-ghost" data-action="show" type="button">Voir la réponse</button>
        </div>
        <div class="tone-keyboard-mount"></div>
        <div class="pinyin-hint-line">Astuce : écris les numéros de ton comme <code>wo3</code> ou clique sur une voyelle accentuée ci-dessous.</div>
      `;
    ...
    mount(ex, host) {
      ...
      host.innerHTML = `
        <div class="syntax-tokens">
          <div class="syntax-build" aria-label="Phrase"></div>
          <div class="syntax-bank-label">Jetons :</div>
          <div class="syntax-bank"></div>
        </div>
        <div class="answer-row">
          <button class="btn btn-accent" data-action="check" type="button">Vérifier</button>
          <button class="btn btn-ghost" data-action="show" type="button">Voir la réponse</button>
          <button class="btn btn-ghost" data-action="reset" type="button">Réinitialiser les jetons</button>
        </div>
      `;
      ...
      function paint() {
        ...
        if (!placed.length) {
          ...
          ph.textContent = "Clique sur les jetons ci-dessous pour construire la phrase.";
...
function renderQuestion() {
  ...
  if (!ex) {
    ...
    promptEl.textContent = "Aucun exercice ne correspond au filtre actuel.";
    stageEl.innerHTML = "";
    activeWidget = null;
    return;
  }
  ...
  if (type === "translate" || type === "syntax") {
    ...
    promptEl.textContent = "Quel sens en français correspond au chinois ci-dessous ?";
  } else if (type === "pinyin") {
    promptEl.textContent = "Écris le pinyin (avec les tons) de ce caractère.";
  }
  stageEl.innerHTML = "";
...
resetBtn.addEventListener("click", () => {
  if (!confirm("Réinitialiser toute la progression des exercices ?")) return;
  remove(RESULTS_KEY);
  remove(SCORE_KEY);
  remove(POS_KEY);
  remove(FILTER_KEY);
