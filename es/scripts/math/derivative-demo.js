...
export function mountDerivativeDemo(rootEl) {
  ...
  const grid = matrixGrid({
    values: [coeffs],
    editable: true,
    caption: "Coeficientes (a₀, a₁, a₂, a₃, a₄)",
    onChange: (vals) => {
      coeffs = vals[0];
      applyCount = 0;
      render();
  ...
  applyBtn.textContent = "Aplicar D";
  ...
  function render({ flash = false } = {}) {
    ...
    statusEl.textContent = dead
      ? ""
      : `Grado actual: ${deg} — el polinomio vive en P${"₀₁₂₃₄"[deg]}.`;

    bannerEl.innerHTML = "";
    if (dead && applyCount > 0) {
      const b = document.createElement("div");
      b.className = "la-banner";
      b.textContent = `D aplicado ${applyCount} veces lo ha borrado todo — y D⁵ anula cualquier polinomio de grado ≤ 4: D es nilpotente de orden 5.`;
