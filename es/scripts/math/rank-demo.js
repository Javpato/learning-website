...
const PRESETS = [
  { label: "Rango 2", values: [[2, 1], [1, 1]] },
  { label: "Rango 1", values: [[1, 1], [2, 2]] },
  { label: "Rango 0", values: [[0, 0], [0, 0]] },
];
...
export function mountRankDemo(rootEl) {
  ...
  const modeButtons = [
    ...
    { key: "compare", label: "Comparar dos matrices del mismo rango" },
  ].map(({ key, label }) => {
  ...
  function renderExplore() {
    ...
    const grid = matrixGrid({
      ...
      editable: true,
      caption: "A — introduce una matriz 2×2",
      onChange: (vals) => {
        matrix = vals;
        render();
      },
    ...
    figCap.textContent =
      r === 2 ? "Rango 2: la imagen conserva toda una superficie."
      : r === 1 ? "Rango 1: la superficie se aplasta sobre una recta."
      : "Rango 0: todo se aplasta en el punto 0.";
    ...
    card.innerHTML = `
      <div class="la-info-line"><span>Rango (dimensión de la imagen)</span><strong>${r}</strong></div>
      <div class="la-info-line"><span>Nulidad (dimensión del núcleo)</span><strong>${2 - r}</strong></div>
    `;
    ...
    canonWrap.appendChild(canonicalForm(r, 2, "Forma canónica buscada").root);
  ...
  function renderCompare() {
    ...
    status.innerHTML =
      "<strong>Mismo rango (1), misma nulidad, misma forma canónica</strong> — pero imágenes diferentes. El mismo «nivel de compresión», no la misma transformación.";
