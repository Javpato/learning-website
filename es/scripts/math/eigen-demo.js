...
export function mountEigenDemo(rootEl) {
  ...
  const left = makePanel("Base canónica");
  const right = makePanel("Base propia");
  ...
  function renderLeft() {
    ...
    left.sub.textContent = current.diagonalizable
      ? "La malla se deforma, pero las direcciones propias permanecen sobre sí mismas."
      : "La dirección e₁ se conserva; todas las demás se mezclan.";
  }
  ...
  function renderRight() {
    ...
    if (current.diagonalizable) {
      ...
      drawSegmentLine(p, { dir: [1, 0], cls: "la-image-line", label: "única dirección propia", labelCls: "success" });
      ...
      warn.textContent = "Una sola dirección propia: no hay base propia — no diagonalizable.";
