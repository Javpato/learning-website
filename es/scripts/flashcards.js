...
function renderTabs() {
  ...
  for (const [id, deck] of Object.entries(decks)) {
    ...
    btn.innerHTML = `${deck.label}<span class="count">${due} pendientes</span>`;
  ...
  if (!items.length) {
    areaEl.innerHTML = `
      <div class="empty-state">
        ...
        <p>Prueba otro mazo de arriba.</p>
      </div>`;
    metaEl.textContent = "";
    return;
  }
...
function renderCard(card, pick) {
  ...
  areaEl.innerHTML = `
    <div class="flashcard-stage">
      <div class="flashcard" id="flashcard">
        <div class="face front">
          ...
          <div class="hint">toca o pulsa espacio</div>
        </div>
        <div class="face back">
          ${backHTML(card)}
        </div>
    ...
    <div class="flashcard-nav">
      ...
      <button class="nav-btn next" type="button">Siguiente (Fácil) →</button>
    </div>
    <div class="rate-row" id="rate-row" style="opacity:0.45; pointer-events:none;">
      <button class="rate-btn again" data-q="1">Otra vez<span class="key">1</span></button>
      <button class="rate-btn hard"  data-q="3">Difícil<span class="key">2</span></button>
      <button class="rate-btn good"  data-q="4">Bien<span class="key">3</span></button>
      <button class="rate-btn easy"  data-q="5">Fácil<span class="key">4</span></button>
    </div>
  `;
  metaEl.innerHTML = pick.mode === "due"
    ? `${pick.dueCount} tarjetas pendientes en este mazo`
    : `Nada pendiente — repasando por adelantado`;
