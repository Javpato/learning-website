...
export function mountPresentation(rootEl, presentation) {
  ...
  rootEl.innerHTML = `
    <div class="pres-chapter-row" role="tablist" aria-label="Capítulos"></div>
    <div class="pres-stage">
      <aside class="pres-side" aria-label="Explicación">
    ...
    <div class="pres-controls">
      ...
      <button class="btn btn-ghost" data-action="play" type="button">▶ Reproducción automática</button>
      <button class="btn btn-accent" data-action="next" type="button">Siguiente →</button>
  ...
  function renderStep() {
    ...
    whyEl.innerHTML = "";
    roadmapEl.innerHTML = "";
    posEl.textContent = `${ch.label} · Paso ${stepIdx + 1} / ${ch.steps.length}`;
    progEl.textContent = `Capítulo ${chapterIdx + 1}/${presentation.chapters.length}`;
    ...
    if (s.whyStep) {
      ...
      sum.textContent = s.whyStep.summary || "Recordatorio — ¿por qué este paso?";
      det.appendChild(sum);
    ...
    if (s.subSteps && s.subSteps.length) {
      ...
      const sum = document.createElement("summary");
      sum.textContent = "¿Por qué? Detallar la transformación";
      det.appendChild(sum);
