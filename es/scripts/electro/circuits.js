...
export function ex4aCircuit({ scenario = "original", emphasis = null, showCurrents = true } = {}) {
  const svg = el("svg", {
    ...
    viewBox: "0 0 520 360",
    role: "img",
    "aria-label": "Circuito de dos ramas RL en paralelo, una con un condensador en serie",
  });
...
export function ex4bCircuit({ emphasis = null } = {}) {
  const svg = el("svg", {
    ...
    viewBox: "0 0 540 240",
    role: "img",
    "aria-label": "Fuente en serie con línea R+L y luego carga Z₂",
  });
...
export function phasorDiagram({ vectors = [], size = 320, scale = 1, title = "" } = {}) {
  ...
  const svg = el("svg", {
    ...
    role: "img",
    "aria-label": title || "Diagrama de Fresnel",
  });
