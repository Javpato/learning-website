...
function diagram({ stage }) {
  ...
  if (stage >= 2 && stage <= 6) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (i === count - 1 && stage <= 5) {
        // top frame, descent
        status = n === 0 ? "= 0 (caso base)" : `= t[${n - 1}] + somme(t, ${n - 1})`;
      } else if (stage === 6 && i === 4) {
        status = "= 0 (caso base)";
      } else {
        status = `en espera`;
  ...
  onStack.forEach((f, i) => {
    ...
    const fr = frame({
      ...
      title: `somme(t, n=${f.n})`,
      slots: [{ label: "retorno", value: f.status, changing: f.status.includes("→") }],
  ...
  const codeLines = [
    "/** @param[in] t arreglo de enteros.",
    " *  @param[in] n número de elementos a sumar. **/",
    "int somme(int t[], int n) {",
    "  if (n == 0) return 0;",
    "  return t[n-1] + somme(t, n-1);",
    "}",
...
const ROADMAP = {
  question: "Suma recursiva: descenso y luego ascenso",
...
export const sommePresentation = {
  chapters: [
    {
      label: "Suma recursiva de un arreglo",
      defaultFigure: (step) => diagram({ stage: step.stage }),
      steps: [
        {
          title: "main: el arreglo T",
          stage: 1,
          note: `El arreglo <code>T = {5, 2, 7, 3}</code> vive en la pila, en el registro de activación de <code>main</code>. <code>s</code> aún no está inicializado.`,
          roadmap: rm(1),
        },
        {
          title: "somme(T, 4) — apilamiento",
          stage: 2,
          note: `El arreglo <code>T</code> se pasa por <strong>decrecimiento en puntero</strong>: la función recibe la dirección de <code>T[0]</code>, no una copia. La prueba <code>n == 0</code> es falsa, así que se evalúa <code>t[3] + somme(t, 3)</code>.`,
          roadmap: rm(2),
        },
        {
          title: "somme(t, 3)",
          stage: 3,
          note: `Nuevo frame, <code>n = 3</code>. El puntero <code>t</code> sigue designando el mismo arreglo en <code>main</code>: no hay copia del arreglo en cada llamada.`,
          roadmap: rm(3),
        },
        {
          title: "somme(t, 2)",
          stage: 4,
          note: `Todavía por encima del caso base. La pila sigue creciendo.`,
          roadmap: rm(4),
        },
        {
          title: "somme(t, 1)",
          stage: 5,
          note: `<code>n = 1</code>, así que se evalúa <code>t[0] + somme(t, 0)</code>. <code>t[0]</code> vale 5: es el valor del arreglo de <code>main</code>, visto a través del puntero.`,
          roadmap: rm(5),
        },
        {
          title: "somme(t, 0) — caso base",
          stage: 6,
          note: `<code>n == 0</code>: se retorna <strong>0</strong>. El descenso se detiene.`,
          roadmap: rm(6),
        },
        {
          title: "Ascenso: somme(t, 1) = 5 + 0",
          stage: 7,
          note: `<code>somme(t, 1)</code> retoma su cálculo: <code>t[0] + 0 = 5 + 0 = 5</code> y retorna.`,
          roadmap: rm(7),
        },
        {
          title: "somme(t, 2) = 2 + 5 = 7",
          stage: 8,
          note: `Recibe 5 de la llamada anterior, suma <code>t[1] = 2</code>. Total: 7.`,
          roadmap: rm(8),
        },
        {
          title: "somme(t, 3) = 7 + 7 = 14",
          stage: 9,
          note: `Recibe 7, suma <code>t[2] = 7</code>. Total: 14.`,
          roadmap: rm(9),
        },
        {
          title: "Retorno final: s = 3 + 14 = 17",
          stage: 10,
          note: `<code>somme(t, 4)</code> suma <code>t[3] = 3</code> a 14. La pila se ha desapilado por completo, <strong>main recibe s = 17</strong>.`,
          roadmap: rm(10),
          whyStep: {
            summary: "Por qué el arreglo no se copia en cada llamada",
            body: `En C, pasar un arreglo a una función se <em>descompone</em> en el paso de un puntero (decay). Todos los frames comparten el mismo arreglo en <code>main</code>: de ahí la flecha discontinua hacia <code>T[0]</code>.`,
          },
        },
      ],
    },
